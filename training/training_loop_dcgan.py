...
def run(
    ...
    model                   = 'dcgan',  # Model to be trained.
    ...
    G = dcgan.Generator(**Config(model=model)(isgen=True)).train().requires_grad_(False).to(device)
    D = dcgan.Discriminator(**Config(model=model)(isgen=False)).train().requires_grad_(False).to(device)
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        with URL.open_url(resume_pkl) as f:
            resume_data = ckpt.load_network_pkl(f, ['G', 'D', 'G_ema'])
    ...
    ddp_modules = dict()
    for name, module in [('G', G), ('D', D), (None, G_ema)]:
    ...
    for name, module in [('G', G), ('D', D)]:
        opt = torch.optim.Adam(module.parameters(), lr=2e-4, betas=(0.5, 0.999))
        phases += [util.EasyDict(name=name+'main', module=module, opt=opt)]
    for phase in phases:
    ...
    while True:
        ...
        if not tensorboard:
            ...
            logger.add_plot(category1='D', category2='G', k='Loss')
            for name, value in stats_metrics.items():
